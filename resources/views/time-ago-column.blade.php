@php
    ...
    if (is_array($arrayState)) {
        ...
        if (!$isListWithLineBreaks && !$isBadge) {
            $arrayState = implode(', ', array_map(fn($value) => $value instanceof \Filament\Support\Contracts\HasLabel ? $value->getLabel() : $value, $arrayState));
        }
    }
...
<div {{ $attributes->merge($getExtraAttributes(), escape: false)->class(['fi-ta-text grid gap-y-1', 'px-3 py-4' => !$isInline()]) }}
    ax-load
